...
import { createNetwork, loadNetworks, type Network } from './network.api.ts'

export default function NetworkCheck() {
  const [organizationId, setOrganizationId] = useState('')
  const [displayName, setDisplayName] = useState('Synthetic Standard Network')
  ...
  async function handleCreate(event: FormEvent) {
    ...
    setError('')
    try {
      await createNetwork(organizationId, displayName)
    } catch (err) {
  ...
  async function handleLoad() {
    ...
    setError('')
    try {
      setNetworks(await loadNetworks(organizationId))
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Request failed')
    } finally {
      setLoading(false)
  ...
  return (
    <section className="mt-6 border-t border-slate-200 pt-6">
      <h2 className="text-lg font-semibold text-slate-900">Network Check</h2>
      ...
      <form className="mt-4 grid gap-2" onSubmit={handleCreate}>
        ...
        <input
          ...
          aria-label="Network display name"
        />
      ...
      {networks && networks.length === 0 && (
        <p className="mt-3 text-sm text-slate-600">No networks yet.</p>
      )}

      {networks && networks.length > 0 && (
        <div className="mt-4 space-y-2">
          <p className="text-sm font-medium text-slate-700">Loaded networks:</p>
          {networks.map((item) => (
